...
- `sourcing-scan`: loads active `sourcing_source_items` + `sourcing_sources`; `http_json` GETs `source_url` and expects JSON (`item_price_usd`, `source_stock_qty`, optional `estimated_shipping_usd`, `estimated_sales_tax_rate_assumed`). Event `items[]` still supported (`custom`) and overrides same `sourcing_source_item_id`. Placeholder types `amazon`/`walmart`/`target` are skipped until implemented. Env: `SOURCING_HTTP_TIMEOUT_SEC`, `SOURCING_HTTP_USER_AGENT`.
