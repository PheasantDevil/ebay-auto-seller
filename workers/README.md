...
- `sourcing-scan`: loads active `sourcing_source_items` + `sourcing_sources`; rows rotate by `last_fetched_at` (oldest first) and the column is updated after each HTTP fetch attempt. `http_json` GETs `source_url` and expects JSON (`item_price_usd`, `source_stock_qty`, optional `estimated_shipping_usd`, `estimated_sales_tax_rate_assumed`). URLs must resolve only to globally routable IPs (SSRF mitigation); optional comma-separated `SOURCING_HTTP_ALLOWED_HOSTS` additionally restricts hostnames. Event `items[]` still supported (`custom`) and overrides same `sourcing_source_item_id` (those rows skip HTTP). Placeholder types `amazon`/`walmart`/`target` are skipped until implemented. Env: `SOURCING_HTTP_TIMEOUT_SEC`, `SOURCING_HTTP_USER_AGENT`, `SOURCING_HTTP_ALLOWED_HOSTS`.
